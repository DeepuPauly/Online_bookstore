 ...
 WHERE tbl_mastcart.cart_status !='Pending' ORDER BY added_date DESC";
               ...
               <?php    }
                ...
                ?>
                                               ...
                                               <?php
$myqry = "select * from tbl_delivery inner join tbl_mastcart using(mastcart_id) where cart_status='assigned to courier' and mastcart_id='$myomif' ";
$myval = select($myqry);
$currentDate = date('Y-m-d');  
if (sizeof($myval)>0) 
{
  $cart_mid=$myval[0]['mastcart_id'];
  $cart_status=$row['ostatus'];
  $Cust_Id=$myval[0]['Cust_Id'];
  $deliver=$myval[0]['Delivery_Id'];
  ...
  ?>
       <td> <p class="text-start text-md-center">
            <strong><a href="?cart_mid=<?php echo $cart_mid; ?>&Cust_Id=<?php echo $Cust_Id; ?>&deliver=<?php echo $deliver; ?>">Reassign</a></strong>
        </p></td>
        <?php
  }
}
